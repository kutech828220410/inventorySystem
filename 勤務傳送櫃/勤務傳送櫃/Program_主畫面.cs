...
namespace 勤務傳送櫃
{
    ...
    {
        private void sub_Program_主畫面_Init()
        {
        ...
        {
            this.sub_Program_主畫面_更新病房資料();
            this.sub_Program_主畫面_讀取RFID();
        }

        #region PLC_主畫面_更新病房資料
        PLC_Device PLC_Device_主畫面_更新病房資料 = new PLC_Device("");
        MyTimer MyTimer_主畫面_更新病房資料_更新間隔 = new MyTimer();
        int cnt_Program_主畫面_更新病房資料 = 65534;
        void sub_Program_主畫面_更新病房資料()
        {
            if (this.plC_ScreenPage_Main.PageText == "主畫面") PLC_Device_主畫面_更新病房資料.Bool = true;
            if (cnt_Program_主畫面_更新病房資料 == 65534)
            {
                PLC_Device_主畫面_更新病房資料.SetComment("PLC_主畫面_更新病房資料");
                PLC_Device_主畫面_更新病房資料.Bool = false;
                cnt_Program_主畫面_更新病房資料 = 65535;
            }
            if (cnt_Program_主畫面_更新病房資料 == 65535) cnt_Program_主畫面_更新病房資料 = 1;
            if (cnt_Program_主畫面_更新病房資料 == 1) cnt_Program_主畫面_更新病房資料_檢查按下(ref cnt_Program_主畫面_更新病房資料);
            if (cnt_Program_主畫面_更新病房資料 == 2) cnt_Program_主畫面_更新病房資料_初始化(ref cnt_Program_主畫面_更新病房資料);
            if (cnt_Program_主畫面_更新病房資料 == 3) cnt_Program_主畫面_更新病房資料_檢查時間到達(ref cnt_Program_主畫面_更新病房資料);
            if (cnt_Program_主畫面_更新病房資料 == 4) cnt_Program_主畫面_更新病房資料_檢查需要寫入SQL(ref cnt_Program_主畫面_更新病房資料);
            if (cnt_Program_主畫面_更新病房資料 == 5) cnt_Program_主畫面_更新病房資料_更新資料(ref cnt_Program_主畫面_更新病房資料);
            if (cnt_Program_主畫面_更新病房資料 == 6) cnt_Program_主畫面_更新病房資料 = 65500;
            if (cnt_Program_主畫面_更新病房資料 > 1) cnt_Program_主畫面_更新病房資料_檢查放開(ref cnt_Program_主畫面_更新病房資料);

            if (cnt_Program_主畫面_更新病房資料 == 65500)
            {
                PLC_Device_主畫面_更新病房資料.Bool = false;
                cnt_Program_主畫面_更新病房資料 = 65535;
            }
        }
        void cnt_Program_主畫面_更新病房資料_檢查按下(ref int cnt)
        {
            if (PLC_Device_主畫面_更新病房資料.Bool) cnt++;
        }
        void cnt_Program_主畫面_更新病房資料_檢查放開(ref int cnt)
        {
            if (!PLC_Device_主畫面_更新病房資料.Bool) cnt = 65500;
        }
        void cnt_Program_主畫面_更新病房資料_初始化(ref int cnt)
        {
            this.MyTimer_主畫面_更新病房資料_更新間隔.StartTickTime(1000);
            cnt++;
        }
        void cnt_Program_主畫面_更新病房資料_檢查時間到達(ref int cnt)
        {
            if(this.MyTimer_主畫面_更新病房資料_更新間隔.IsTimeOut())
            {
                cnt++;
            } 
        }
        void cnt_Program_主畫面_更新病房資料_檢查需要寫入SQL(ref int cnt)
        {
        ...
        void cnt_Program_主畫面_更新病房資料_更新資料(ref int cnt)
        {
        ...
        #endregion
        #region PLC_主畫面_讀取RFID
        PLC_Device PLC_Device_主畫面_讀取RFID = new PLC_Device("");
        int cnt_Program_主畫面_讀取RFID = 65534;
        void sub_Program_主畫面_讀取RFID()
        {
            if (this.plC_ScreenPage_Main.PageText != "人員權限") PLC_Device_主畫面_讀取RFID.Bool = true;
            else PLC_Device_主畫面_讀取RFID.Bool = false;
            if (cnt_Program_主畫面_讀取RFID == 65534)
            {
                PLC_Device_主畫面_讀取RFID.SetComment("PLC_主畫面_讀取RFID");
                PLC_Device_主畫面_讀取RFID.Bool = false;
                cnt_Program_主畫面_讀取RFID = 65535;
            }
            if (cnt_Program_主畫面_讀取RFID == 65535) cnt_Program_主畫面_讀取RFID = 1;
            if (cnt_Program_主畫面_讀取RFID == 1) cnt_Program_主畫面_讀取RFID_檢查按下(ref cnt_Program_主畫面_讀取RFID);
            if (cnt_Program_主畫面_讀取RFID == 2) cnt_Program_主畫面_讀取RFID_初始化(ref cnt_Program_主畫面_讀取RFID);
            if (cnt_Program_主畫面_讀取RFID == 3) cnt_Program_主畫面_讀取RFID = 65500;
            if (cnt_Program_主畫面_讀取RFID > 1) cnt_Program_主畫面_讀取RFID_檢查放開(ref cnt_Program_主畫面_讀取RFID);

            if (cnt_Program_主畫面_讀取RFID == 65500)
            {
                PLC_Device_主畫面_讀取RFID.Bool = false;
                cnt_Program_主畫面_讀取RFID = 65535;
            }
        }
        void cnt_Program_主畫面_讀取RFID_檢查按下(ref int cnt)
        {
            if (PLC_Device_主畫面_讀取RFID.Bool) cnt++;
        }
        void cnt_Program_主畫面_讀取RFID_檢查放開(ref int cnt)
        {
            if (!PLC_Device_主畫面_讀取RFID.Bool) cnt = 65500;
        }
        void cnt_Program_主畫面_讀取RFID_初始化(ref int cnt)
        {
            string IP = "";
            int RFID_Num = -1;
            string 卡號 = "";
