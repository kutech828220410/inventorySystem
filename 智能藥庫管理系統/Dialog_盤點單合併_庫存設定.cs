...
namespace 智能藥庫系統
{
    public partial class Dialog_盤點單合併_庫存設定 : MyDialog
    {
        private inv_combinelistClass inv_CombinelistClass = null;
        private string _SN;
        public Dialog_盤點單合併_庫存設定(string SN)
        {
            form.Invoke(new Action(delegate 
            {
                InitializeComponent();
                Table table = new Table(new enum_inv_combinelist_stock());
                this.sqL_DataGridView_庫存設定表.RowsHeight = 50;
                this.sqL_DataGridView_庫存設定表.Init(table);
                this.sqL_DataGridView_庫存設定表.Set_ColumnVisible(false, new enum_inv_combinelist_stock().GetEnumNames());
                this.sqL_DataGridView_庫存設定表.Set_ColumnWidth(100, DataGridViewContentAlignment.MiddleCenter, enum_inv_combinelist_stock.藥碼);
                this.sqL_DataGridView_庫存設定表.Set_ColumnWidth(550, DataGridViewContentAlignment.MiddleLeft, enum_inv_combinelist_stock.藥名);
                this.sqL_DataGridView_庫存設定表.Set_ColumnWidth(100, DataGridViewContentAlignment.MiddleCenter, enum_inv_combinelist_stock.數量);
                this.sqL_DataGridView_庫存設定表.Set_ColumnText("庫存", enum_inv_combinelist_stock.數量);
            }));
            ...
            this.LoadFinishedEvent += Dialog_盤點單合併_庫存設定_LoadFinishedEvent;
            _SN = SN;
        }
        ...
        private void RefreshUI()
        {
            ...
            List<object[]> list_inv_combinelist_stock_Class = inv_CombinelistClass.Stocks.ClassToSQL<inv_combinelist_stock_Class, enum_inv_combinelist_stock>();
            this.sqL_DataGridView_庫存設定表.RefreshGrid(list_inv_combinelist_stock_Class);
        ...
        private void PlC_RJ_Button_匯出_MouseDownEvent(MouseEventArgs mevent)
        {
            this.Invoke(new Action(delegate 
            {
                ...
                DataTable dataTable = sqL_DataGridView_庫存設定表.GetAllRows().ToDataTable(new enum_inv_combinelist_stock());
                dataTable.ColumnRename("數量", "庫存");
                dataTable.RemoveColumn(enum_inv_combinelist_stock.GUID);
                dataTable.RemoveColumn(enum_inv_combinelist_stock.合併單號);
                dataTable.RemoveColumn(enum_inv_combinelist_stock.加入時間);
                dataTable.NPOI_SaveFile(this.saveFileDialog_SaveExcel.FileName, new string[] { "庫存" });
