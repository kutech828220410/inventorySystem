...
namespace 調劑台管理系統
{
    public partial class Dialog_調劑作業_病歷號輸入 : Form
    {
        ...
        private void Dialog_調劑作業_病歷號輸入_Load(object sender, EventArgs e)
        {
            ...
            this.sqL_DataGridView_醫囑資料.Set_ColumnWidth(100, DataGridViewContentAlignment.MiddleLeft, enum_醫囑資料.藥品碼);
            ...
            this.sqL_DataGridView_醫囑資料_已選取處方.Init(table);
            this.sqL_DataGridView_醫囑資料_已選取處方.Set_ColumnVisible(false, new enum_醫囑資料().GetEnumNames());
            this.sqL_DataGridView_醫囑資料_已選取處方.Set_ColumnWidth(100, DataGridViewContentAlignment.MiddleLeft, enum_醫囑資料.藥品碼);
            this.sqL_DataGridView_醫囑資料_已選取處方.Set_ColumnWidth(250, DataGridViewContentAlignment.MiddleLeft, enum_醫囑資料.藥品名稱);
            this.sqL_DataGridView_醫囑資料_已選取處方.Set_ColumnWidth(60, DataGridViewContentAlignment.MiddleRight, enum_醫囑資料.交易量);
            this.sqL_DataGridView_醫囑資料_已選取處方.Set_ColumnWidth(120, DataGridViewContentAlignment.MiddleCenter, enum_醫囑資料.開方日期);
        ...
        {
            List<object[]> list_已選取處方 = sqL_DataGridView_醫囑資料_已選取處方.GetAllRows();
            List<object[]> list_已選取處方_buf = new List<object[]>();
            List<object[]> list_醫囑資料 = this.sqL_DataGridView_醫囑資料.Get_All_Select_RowsValues();
            if(list_醫囑資料.Count == 0)
            {
                MyMessageBox.ShowDialog("未選取處方資料!");
                return;
            }
            string GUID = list_醫囑資料[0][(int)enum_醫囑資料.GUID].ObjectToString();
            ...
            sqL_DataGridView_醫囑資料_已選取處方.AddRow(list_醫囑資料[0], true);
        }
        private void RJ_Button_刪除_MouseDownEvent(MouseEventArgs mevent)
        {
            List<object[]> list_已選取處方 = sqL_DataGridView_醫囑資料_已選取處方.Get_All_Select_RowsValues();
            ...
            sqL_DataGridView_醫囑資料_已選取處方.DeleteExtra(list_已選取處方, true);
        ...
        private void RJ_Button_確認_MouseDownEvent(MouseEventArgs mevent)
        {
            this.Invoke(new Action(delegate
            {
                List<object[]> list_已選處方 = sqL_DataGridView_醫囑資料_已選取處方.GetAllRows();
        ...
        public class ICP_醫囑資料 : IComparer<object[]>
        {
            //實作Compare方法
            //依Speed由小排到大。
